(ns metabase.sync.sync-metadata
  "Logic responsible for syncing the metadata for an entire database.
   ...
   4.  Sync Kenga Analytics Metadata table (`metabase.sync.sync-metadata.metabase-metadata`)"
...
(s/defn sync-db-metadata!
  "Sync the metadata for a Kenga Analytics `database`. This makes sure child Table & Field objects are synchronized."
