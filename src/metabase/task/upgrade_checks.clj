(ns metabase.task.upgrade-checks
  "Contains a Kenga Analytics task which periodically checks for the availability of new Kenga Analytics versions."
...
(jobs/defjob CheckForNewVersions [_]
  (when (public-settings/check-for-updates)
    (log/debug (trs "Checking for new Kenga Analytics version info."))
    (try
