...
(defn- abandonment-context []
  {:heading      (str (trs "We’d love your feedback."))
   :callToAction (str (trs "It looks like Kenga Analytics wasn’t quite a match for you.")
                      " "
                      (trs "Would you mind taking a fast 5 question survey to help the Kenga Analytics team understand why and make things better in the future?"))
   :link         "https://www.metabase.com/feedback/inactive"})

(defn- follow-up-context []
  {:heading      (str (trs "We hope you''ve been enjoying Kenga Analytics."))
...
(defn send-new-user-email!
  ...
  (let [company      (or (public-settings/site-name) "Unknown")
    ...
    (email/send-message!
      :subject      (str "You're invited to join " company "")
...
(defn send-password-reset-email!
  ...
  (let [message-body (stencil/render-file "metabase/email/password_reset"
                       {:emailType        "password_reset"
                        ...
                        :logoHeader       true})]
    (email/send-message!
      :subject      (str (trs "[Kenga Analytics] Password Reset Request"))
      :recipients   [email]
      :message-type :html
      :message      message-body)))
...
(defn send-notification-email!
  ...
  (let [context      (merge (update context :dependencies build-dependencies)
    ...
    (email/send-message!
      :subject      (str (trs "[Kenga Analytics] Notification"))
      :recipients   [email]
      :message-type :html
      :message      message-body)))
